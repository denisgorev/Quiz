...
import com.quiz_app_den_hfad.quiz.adapters.QuizAdapter;
import com.quiz_app_den_hfad.quiz.constans.AppConstans;
import com.quiz_app_den_hfad.quiz.data.preference.AppPreference;
import com.quiz_app_den_hfad.quiz.listeners.ListitemClickListener;
import com.quiz_app_den_hfad.quiz.models.quiz.QuizModel;
import com.quiz_app_den_hfad.quiz.utilities.ActivityUtilities;
import com.quiz_app_den_hfad.quiz.utilities.BeatBox;
import com.quiz_app_den_hfad.quiz.utilities.DialogUtilities;
import com.quiz_app_den_hfad.quiz.utilities.SoundUtilities;
...
public class QuizActivity extends BaseActivity implements  DialogUtilities.OnCompleteListener {
    ...
    private void initVar() {
        ...
        if (intent != null) {
            mCategoryId = intent.getStringExtra(AppConstans.BUNDLE_KEY_INDEX);
        }
    ...
    private void loadData() {
        ...
        isSoundOn = AppPreference.getInstance(mActivity).getBoolean(AppConstans.KEY_SOUND, true);
        setSpeakerImage();
    ...
    public void initListener() {
        btnSpeaker.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                isSoundOn = !isSoundOn;
                AppPreference.getInstance(mActivity).setBoolean(AppConstans.KEY_SOUND, isSoundOn);
                setSpeakerImage();
            }
        });
        ...
        btnNext.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!mUserHasPressed) {
                    ...
                    DialogUtilities dialog = DialogUtilities.newInstance(getString(R.string.skip_text), getString(R.string.skip_prompt), getString(R.string.yes), getString(R.string.no), AppConstans.BUNDLE_KEY_SKIP_OPTION);
                    dialog.show(manager, AppConstans.BUNDLE_KEY_DIALOG_FRAGMENT);
                } else {
                    ...
                    if (mItemList.get(mQuestionPosition).getCorrectAnswer() != -1) {
                        for (int currentItemIndex = 0; currentItemIndex < mOptionList.size(); currentItemIndex++) {
                            if (currentItemIndex == clickedAnswerIndex && currentItemIndex == mItemList.get(mQuestionPosition).getCorrectAnswer()) {
                                mBackgroundColorList.set(currentItemIndex, AppConstans.COLOR_GREEN);
                                mScore++;
                                mIsCorrect = true;
                                if (isSoundOn) {
                                    mBeatBox.play(mSounds.get(AppConstans.BUNDLE_KEY_ZERO_INDEX));
                                }
                            } else if (currentItemIndex == clickedAnswerIndex && !(currentItemIndex == mItemList.get(mQuestionPosition).getCorrectAnswer())) {
                                mBackgroundColorList.set(currentItemIndex, AppConstans.COLOR_RED);
                                mWrongAns++;
                                if (isSoundOn) {
                                    mBeatBox.play(mSounds.get(AppConstans.BUNDLE_KEY_SECOND_INDEX));
                                }
                        ...
                        mBackgroundColorList.set(clickedAnswerIndex, AppConstans.COLOR_GREEN);
                        mScore++;
                        mIsCorrect = true;
                        mBeatBox.play(mSounds.get(AppConstans.BUNDLE_KEY_ZERO_INDEX));
                    }
    ...
    void increaseLifeAndStatus() {
        if (mLifeCounter < AppConstans.BUNDLE_KEY_MAX_LIFE) {
            mLifeCounter++;
            setLifeStatus();
        }
    }
    ...
    public void setNextQuestion() {
        if (isSoundOn) {
            mBeatBox.play(mSounds.get(AppConstans.BUNDLE_KEY_FIRST_INDEX));
        }
        ...
        if (mQuestionPosition < mItemList.size() - 1 && mLifeCounter > 0) {
            ...
            FragmentManager manager = getSupportFragmentManager();
            DialogUtilities dialog = DialogUtilities.newInstance(getString(R.string.reward_dialog_title), getString(R.string.reward_dialog_message), getString(R.string.yes), getString(R.string.no), AppConstans.BUNDLE_KEY_REWARD_OPTION);
            dialog.show(manager, AppConstans.BUNDLE_KEY_DIALOG_FRAGMENT);
        } else {
    ...
    public void updateQuestionsAndAnswers() {
        ...
        ((LinearLayoutManager) mRecyclerQuiz.getLayoutManager()).scrollToPosition(AppConstans.BUNDLE_KEY_ZERO_INDEX);
    ...
    public void quizActivityClosePrompt() {
        FragmentManager manager = getSupportFragmentManager();
        DialogUtilities dialog = DialogUtilities.newInstance(getString(R.string.exit), getString(R.string.cancel_prompt), getString(R.string.yes), getString(R.string.no), AppConstans.BUNDLE_KEY_CLOSE_OPTION);
        dialog.show(manager, AppConstans.BUNDLE_KEY_DIALOG_FRAGMENT);
    }
    ...
    private void loadJson() {
        ...
        try {
            br = new BufferedReader(new InputStreamReader(getAssets().open(AppConstans.QUESTION_FILE)));
            String temp;
    ...
    public void parseJson(String jsonData) {
        try {
            ...
            JSONArray jsonArray = jsonObjMain.getJSONArray(AppConstans.JSON_KEY_QUESTIONNAIRY);

            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject jsonObj = jsonArray.getJSONObject(i);

                String question = jsonObj.getString(AppConstans.JSON_KEY_QUESTION);
                int correctAnswer = Integer.parseInt(jsonObj.getString(AppConstans.JSON_KEY_CORRECT_ANS));
                String categoryId = jsonObj.getString(AppConstans.JSON_KEY_CATEGORY_ID);

                Log.d("TAG", categoryId.toString());

                JSONArray jsonArray2 = jsonObj.getJSONArray(AppConstans.JSON_KEY_ANSWERS);
                ...
                for (int j = 0; j < jsonArray2.length(); j++) {
                    ...
                    backgroundColors.add(AppConstans.COLOR_WHITE);
                }
    ...
    public void onComplete(Boolean isOkPressed, String viewIdText) {
        if (isOkPressed) {
            if (viewIdText.equals(AppConstans.BUNDLE_KEY_CLOSE_OPTION)) {
                ActivityUtilities.getInstance().invokeNewActivity(mActivity, MainActivity.class, true);
            } else if (viewIdText.equals(AppConstans.BUNDLE_KEY_SKIP_OPTION)) {
                mSkip++;
                ...
                //TODO: updateResultSet();
                setNextQuestion();
            } else if (viewIdText.equals(AppConstans.BUNDLE_KEY_REWARD_OPTION)) {
                //TODO:  mRewardedVideoAd.show();
            }
        } else if (!isOkPressed && viewIdText.equals(AppConstans.BUNDLE_KEY_REWARD_OPTION)) {
            //TODO: invoke ScoreCardActivity
